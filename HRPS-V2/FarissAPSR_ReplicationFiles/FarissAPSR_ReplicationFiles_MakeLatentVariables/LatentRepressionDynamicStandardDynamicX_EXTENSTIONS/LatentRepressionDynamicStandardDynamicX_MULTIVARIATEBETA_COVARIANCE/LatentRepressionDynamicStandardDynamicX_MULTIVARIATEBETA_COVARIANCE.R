...
MakeInits <- function(){
  ...
  #out <- list(mu=MU, alpha1=ALPHA1, alpha03=ALPHA03, alpha05=ALPHA05,  alpha06=ALPHA06, beta1=BETA1, beta3=BETA3, beta5=BETA5, beta6=BETA6, sigma=SIGMA, b0=b0, B0=B0)
  out <- list(mu=MU, alpha1=ALPHA1, alpha03=ALPHA03, alpha05=ALPHA05,  alpha06=ALPHA06, sigma=SIGMA, B=B)
  return(out)
}
inits1 <- MakeInits()
inits2 <- MakeInits()
...
NAME <- "LatentRepressionDynamicStandardDynamicX_MULTIVARIATEBETA_COVARIANCE.bug"

# print time taken
print(Sys.time() - systime1)
  
#rjags code version
m <- jags.model(file=NAME, data=list("y"=y, "time"=time, "year"=year, "country"=country, "n.country"=length(panel), "n.year"=max(panel), "n"=nrow(y), "b0"=b0, "B0"=B0), inits=inits.function, n.chains=CHAINS, n.adapt=ADAPT)
...
M <- coda.samples(m, variable.names=c("x", "beta1", "beta3", "beta5", "beta6", "alpha1", "alpha3", "alpha5", "alpha6", "kappa", "sigma", "OMEGA"), n.iter=DRAWS, progress.bar="text", thin=THIN)
...
write.csv(as.data.frame(posterior.estimates), "EstimateDynamicStandardDynamicX_MULTIVARIATEBETA_COVARIANCE.csv", row.names=F)
