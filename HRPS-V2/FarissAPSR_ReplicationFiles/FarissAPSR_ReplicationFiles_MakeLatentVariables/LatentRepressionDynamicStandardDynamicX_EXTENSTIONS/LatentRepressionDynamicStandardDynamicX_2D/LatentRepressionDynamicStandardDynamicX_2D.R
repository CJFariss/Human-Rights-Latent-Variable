...
MakeInits <- function(){
  ...
  MU1 <- matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel))
  MU2 <- matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel))

  GAMMA1 <- runif(nIRT.Binary)
  BETA1 <- runif(nIRT.Binary)
  ALPHA1 <- runif(nIRT.Binary)

  GAMMA3 <- runif(nIRT.Ordered3)
  ...
  out <- list(mu1=MU1, mu2=MU2, alpha1=ALPHA1, alpha03=ALPHA03, alpha05=ALPHA05,  alpha06=ALPHA06, beta1=BETA1, beta3=BETA3, beta5=BETA5, beta6=BETA6, gamma1=GAMMA1, gamma3=GAMMA3, gamma5=GAMMA5, gamma6=GAMMA6, sigma1=SIGMA1, sigma2=SIGMA2)
  return(out)
}
inits1 <- MakeInits()
inits2 <- MakeInits()
...
NAME <- "LatentRepressionDynamicStandardDynamicX_2D.bug"
...
M <- coda.samples(m, variable.names=c("x1", "x2", "gamma1", "gamma3", "gamma5", "gamma6", "beta1", "beta3", "beta5", "beta6", "alpha1", "alpha3", "alpha5", "alpha6", "kappa1", "sigma1", "kappa2", "sigma2"), n.iter=DRAWS, progress.bar="text", thin=THIN)
