...
inits.function <- function(chain){
  return(switch(chain,
         "1"=list(mu=matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel)), beta=runif(7), 
alpha0=matrix(c(runif(7), runif(7, 1, 2)), nrow=7, ncol=2, byrow=TRUE), sigma=runif(1)),
         "2"=list(mu=matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel)), beta=runif(7), alpha0=matrix(c(runif(7), runif(7, 1, 2)), nrow=7, ncol=2, byrow=TRUE), sigma=runif(1)),
         "3"=list(mu=matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel)), beta=runif(7), alpha0=matrix(c(runif(7), runif(7, 1, 2)), nrow=7, ncol=2, byrow=TRUE), sigma=runif(1))))
}
