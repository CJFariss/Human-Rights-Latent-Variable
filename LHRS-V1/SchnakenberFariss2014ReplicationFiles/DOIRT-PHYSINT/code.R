...
inits.function <- function(chain){
  return(switch(chain,
         "1"=list(mu=matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel)), beta=runif(4), alpha0=matrix(c(runif(4), runif(4, 1, 2)), nrow=4, ncol=2, byrow=TRUE), sigma=runif(1)),
         "2"=list(mu=matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel)), beta=runif(4), alpha0=matrix(c(runif(4), runif(4, 1, 2)), nrow=4, ncol=2, byrow=TRUE), sigma=runif(1)),
         "3"=list(mu=matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel)), beta=runif(4), alpha0=matrix(c(runif(4), runif(4, 1, 2)), nrow=4, ncol=2, byrow=TRUE), sigma=runif(1))))
}
#source("draws1.R")
#source("draws2.R")
#source("draws3.R")
...
#rjags code version
m <- jags.model(file="model.bug", data=list("y"=y, "year"=year, "country"=country, "n.country"=length(panel), "n.year"=max(panel), "n"=nrow(y)), inits=inits.function, n.chains=3, n.adapt=ADAPT)
update(m, BURNIN)
M <- coda.samples(m, variable.names=c("x", "beta", "alpha", "kappa", "sigma"), n.iter=DRAWS, progress.bar="text", thin=THIN)
